...
usage: |
  ...
  Usage: generate_diagram.py [commit-ish]
  Default: HEAD
  
  Output: output/diagram_<commit>.dot (and .png if Graphviz available)
tags:
  - diagram
  - visualization
  - code-review
...
- **Diagram**: `output/diagram_<commit>.dot`
- **PNG**: `output/diagram_<commit>.png` (if dot available)

## Example

```bash
python skills/generate-pr-diagram/generate_diagram.py HEAD
```
