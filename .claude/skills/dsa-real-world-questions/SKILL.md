...
2. **"Answers & Approach"** — the same questions repeated (same numbering), each followed by:
   - **Concept:** which data structure/algorithm applies
   - **Why:** 1-3 sentences connecting the scenario's constraints to why that concept fits
     (e.g. "O(1) average lookup for a huge, unordered user base rules out a sorted array or
     tree; a hash table's the natural fit")
